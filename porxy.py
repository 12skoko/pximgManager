...
def parse_ssr(base64_encode_str):
    ...
    print("解析结果:")
    print('server:%s,port:%s,协议:%s,加密方法:%s,密码:%s,混淆:%s,混淆参数:%s,协议参数:%s,备注:%s,分组:%s'%(server, port, protocol, method, password, obfs, obfsparam, protoparam, remarks, group ))
...
def parse():
    while True:
        ...
        if ssr.startswith('ss://'):
            base64_encode_str=ssr[5:1]
            parse_ss(base64_encode_str)
            continue

        if ssr.startswith('ssr://'):
            base64_encode_str=ssr[6:1]
